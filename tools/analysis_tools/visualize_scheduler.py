...
class SimpleModel(BaseModel):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = MODELS.build(cfg.model.backbone)
...
class ParamRecordHook(Hook):
    ...
    def after_train_iter(self, runner, batch_idx, data_batch, outputs):
        ...
        num_lr = len(runner.optim_wrapper.get_lr()['lr'])
        if self.first:
            self.lr_list = [[] for _ in range(num_lr)]
            self.first = False

        for i in range(num_lr):
            self.lr_list[i].append(runner.optim_wrapper.get_lr()['lr'][i])
