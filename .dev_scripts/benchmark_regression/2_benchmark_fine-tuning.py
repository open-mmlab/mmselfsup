...
def parse_args():
    ...
    parser.add_argument(
        '--work-dir',
        default='work_dirs/benchmark_fine_tuning',
...
def create_train_job_batch(commands, model_info, args, port, script_name):
    ...
    gpus = model_info.data['FinetuningGPUs']

    config_path = model_info.data.get('FinetuningConfig', None)
    if config_path:
        config = Path(config_path)
        assert config.exists(), f'"{fname}": {config} not found.'

        weights = model_info.data.get('Weights', None)
        assert weights is not None, 'Failed to get pretrained weights.'
        ...
        job_script = (f'#!/bin/bash\n'
                      ...
                      f'checkpoint_config.max_keep_ckpts=3 '
                      f'--launcher={launcher}\n')
