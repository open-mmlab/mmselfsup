...
class GreenMIMNeck(BaseModule):
    ...
    def __init__(self, in_channels: int, encoder_stride: int, img_size, patch_size,
        embed_dim=96, depths=[2, 2, 6, 2], decoder_embed_dim=512,
        mlp_ratio=4., decoder_depth=8, decoder_num_heads=16, block_cls=Block) -> None:
        super().__init__()
        ...
        self.decoder_blocks = nn.ModuleList([
            block_cls(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, norm_layer=torch.nn.LayerNorm)
            for i in range(decoder_depth)])

        self.decoder_norm = torch.nn.LayerNorm(decoder_embed_dim)
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_channels, bias=True) # encoder to decoder
